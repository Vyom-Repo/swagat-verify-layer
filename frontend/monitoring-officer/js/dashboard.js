...
document.getElementById('statusFilter').addEventListener('change', () => {
  ...
  chipsEl.querySelectorAll('.filter-chip').forEach(c => c.classList.toggle('active', c.textContent === (val || 'All')));
  renderTable();
});
...
function renderAnalytics() {
  ...
  const bestOfficer = Object.entries(officers).sort((a, b) => b[1].approved - a[1].approved)[0];
  ...
  const topArea = Object.entries(areas).sort((a, b) => b[1].total - a[1].total)[0];
  ...
  new Chart(document.getElementById('chartMonthly'), {
    ...
    data: {
      ...
      datasets: [
        { label: 'Received', data: [3, 5, 17, 0], backgroundColor: '#3B82F6', borderRadius: 6 },
        { label: 'Resolved', data: [2, 3, 10, 0], backgroundColor: '#10B981', borderRadius: 6 }
      ]
    },
